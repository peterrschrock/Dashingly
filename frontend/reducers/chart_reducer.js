import {RECEIVE_CHART_TYPE, RECEIVE_CHART_TITLE, RECEIVE_X_AXIS_TITLE, RECEIVE_Y_AXIS_TITLE, RECEIVE_DATA_ID, RECEIVE_X_DATA, RECEIVE_Y_DATA} from '../actions/chart_actions';
...
const ChartReducer = (state = _noCharts, action) => {
  ...
  switch (action.type) {
    ...
    case RECEIVE_X_AXIS_TITLE:
      let newXAxisChart = merge({}, state.chartNewState, {x_axis_title: action.x_axis});
      return merge({}, state, {chartNewState: newXAxisChart});
    case RECEIVE_Y_AXIS_TITLE:
      let newYAxisChart = merge({}, state.chartNewState, {y_axis_title: action.y_axis});
      return merge({}, state, {chartNewState: newYAxisChart});
    case RECEIVE_DATA_ID:
      let newDataId = merge({}, state.chartNewState, {data_id: action.data_id});
